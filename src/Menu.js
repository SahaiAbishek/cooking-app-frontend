...
class Menu extends Component {
    ...
    componentDidMount() {
        axios.get(`http://boiling-hamlet-20361.herokuapp.com/cooking/food/items`)
            .then(response => {
    ...
    searchFood(event) {
        if (event.key === 'Enter') {
            axios.get(`http://boiling-hamlet-20361.herokuapp.com/cooking/food/items/` + event.target.value)
                .then(response => {
                    const menuItems = response.data;
                    this.setState({ menuItems });
                }).catch(function (error) {
    ...
    updateFoodPic(item) {
        const url = 'http://boiling-hamlet-20361.herokuapp.com/cooking/food/ID/' + item.id + '/'
        console.log(url);
