...
class Menu extends Component {
    ...
    componentDidMount() {
        axios.get(`http://localhost:12345/cooking/food/items`)
            .then(response => {
    ...
    searchFood(event) {
        if (event.key === 'Enter') {
            axios.get(`http://localhost:12345/cooking/food/items/` + event.target.value)
                .then(response => {
                    const menuItems = response.data;
                    this.setState({ menuItems });
                }).catch(function (error) {
    ...
    updateFoodPic(item) {
        const url = 'http://localhost:12345/cooking/food/ID/' + item.id + '/'
        console.log(url);
